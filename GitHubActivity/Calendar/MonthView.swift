...
struct MonthView<Accessory: View>: View {
    ...
    init(month: Date, selection: CalendarSelection, @ViewBuilder accessoryBuilder: @escaping (_ date: Date) -> Accessory) {
        self.month = month
...
extension MonthView where Accessory == EmptyView {
    init(month: Date, selection: CalendarSelection) {
        self.month = month
        self.selection = selection
...
struct MonthView_Previews : PreviewProvider {
    static var previews: some View {
        Group {
            MonthView(month: Date().addingTimeInterval(60*60*24*365*7), selection: CalendarSelection())
            MonthView(month: Date().addingTimeInterval(60*60*24*365*7), selection: CalendarSelection()) { date in
                Circle()
